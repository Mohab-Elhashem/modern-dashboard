                    ...
                    list.map((item)=>(
                        <Card key={item.id} className="flex-row items-center justify-between gap-4 p-4">
                            <div className="w-12 h-12 rounded-sm relative overflow-hidden">
                                <Image src={item.image} alt={item.title} fill className="object-cover" sizes="10"/>
                            </div>
                            <CardContent className="p-0 flex-1">
                                <CardTitle className="text-sm font-medium">{item.title}</CardTitle>
                                <Badge variant="outline" className="rounded-xl mb-2">{item.badge}</Badge>
                            </CardContent>
                            <CardFooter className="bg-transparent border-0">{item.count / 1000}K</CardFooter>
                        </Card>
                    ))
                }
            </div>
